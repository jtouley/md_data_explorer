...
class TestSurvivalAnalysis:
    ...
    def test_run_kaplan_meier_single_cohort(self):
        ...
        kmf, summary = run_kaplan_meier(df, duration_col="duration", event_col="event")
    ...
    def test_run_kaplan_meier_with_nulls(self):
        ...
        df = pd.DataFrame({"duration": [10, 20, None, 40, 50], "event": [1, None, 0, 1, 1]})
    ...
    def test_run_kaplan_meier_all_nulls(self):
        ...
        df = pd.DataFrame({"duration": [None, None, None], "event": [None, None, None]})
    ...
    def test_run_cox_regression(self):
        ...
        cph, summary_df = run_cox_regression(
            df, duration_col="duration", event_col="event", covariates=["age", "treatment"]
        )
    ...
    def test_run_logrank_test_two_groups(self):
        ...
        result = run_logrank_test(df, duration_col="duration", event_col="event", group_col="group")
    ...
    def test_run_logrank_test_single_group(self):
        ...
        df = pd.DataFrame({"duration": [10, 20, 30], "event": [1, 1, 0], "group": ["A", "A", "A"]})

        with pytest.raises(ValueError, match="Need at least 2 groups"):
            run_logrank_test(df, duration_col="duration", event_col="event", group_col="group")
