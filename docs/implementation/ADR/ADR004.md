...
**Goal**: Provide a bounded, privacy-safe context pack to ADR001's Tier 3 LLM fallback, enabling grounded query parsing for negation, multi-part queries, and codebook-driven columns.

**Problem**: Production logs show Tier 3 returns `DESCRIBE, confidence=0.3, matched_vars=[]` for queries like "average BMI" / "average LDL" because the model has no schema cues. This produces "COUNT with no variables" and "group by patient_id" failures (see ADR003 Production Contract Violations).

**Solution**: AutoContext Pack provides structured, privacy-safe dataset context to Tier 3, enabling it to:
- Map user terminology ("BMI", "LDL") to canonical column names
- Understand codebook encodings ("1: Yes 2: No" → categorical)
- Identify entity keys (patient_id) to avoid grouping on them
- Extract filters with negation ("not on statin" → filter condition)
- Handle multi-part queries with proper variable selection
...
- Trust UI must display the exact AutoContext summary passed to Tier 3
...
```
System: "Return only JSON matching QueryPlan schema."

Provide:
- The query text
- The AutoContext pack (structured JSON)
- Allowed operators / intents list
- QueryPlan schema with required fields

Require fields:
- intent
- primary_variable or grouping_variable when intent needs it
- explicit filters list with negation encoded structurally
...
**Rationale**: Structured prompt with schema constraints enables Tier 3 to:
- Map "BMI" → canonical column name using aliases
- Understand "1: Yes 2: No" → categorical codebook
- Identify `patient_id` as entity_key → avoid grouping on it
- Extract "not on statin" → filter condition using codebook values

This directly addresses production failures where Tier 3 returns plans with no variables or wrong grouping.
...
def build_autocontext(
    ...
    Returns:
        AutoContext pack (privacy-safe, bounded)
    """
    # Implementation:
    # 1. Extract entity keys from inferred_schema (patient_id, etc.)
    # 2. Build column catalog from semantic layer:
    #    - Get canonical columns
    #    - Get system aliases (from alias index)
    #    - Get user aliases (from ADR003 persisted mappings)
    #    - Get inferred types (from schema inference)
    #    - Get codebooks (from column name patterns: "1: Yes 2: No")
    #    - Get lightweight stats (min/max/mean for numeric, top N for categorical)
    # 3. Extract glossary from doc_context (abbreviations, definitions)
    # 4. Filter columns by relevance if query_terms provided
    # 5. Enforce token budget (truncate if needed)
    # 6. Validate no row-level data included
    pass
```
...
**Success Criteria**:
- [ ] AutoContext pack built from schema inference + documentation + aliases
...
- [ ] Tier 3 produces valid QueryPlans with variables (not `matched_vars=[]`)
