...
- **Show Effective Execution**:
  ...
  - **Effective filters** (post-normalization, with actual values):
    ...
    - Show normalized filter values (e.g., "Current Regimen IN [1,2,3,4,5,6,7,8,9]" → label as "non-restrictive" if all valid codes)
  - **Cohort size**:
...
- **Validate QueryPlan Contract**:
  ...
  - **COUNT-specific validation**:
    ...
    - **Breakdown validation**: 
      ...
      - If `grouping_variable` is high-cardinality (near-unique, e.g., patient_id with ~N unique values where N ≈ total rows) and query implies categorical grouping, refuse with error "Grouping by [entity_key] yields ~[N] groups. Choose a categorical column (e.g., Current Regimen)."
...
- Tier 3: LLM fallback (Ollama integration with local model) ✅
...
- **Model recommendation** (if implementing):
  ...
  - **Note**: Model file size ≠ RAM usage (depends on quantization, KV cache, context length, batch size)
