...
- Adaptive alias persistence (user teaches system terminology → persists to metadata)
...
2. **Filter extraction not implemented**: No code exists to parse "not on statin" → filter condition
...
- **Enhanced parsing** (ADR003 Phase 2): System needs structured LLM extraction to handle negation and map "LDL" → "LDL mg/dL"
...
- **Query intent understanding**: System must distinguish between:
  - "how many X?" → COUNT with filter
  - "describe X" → DESCRIBE with statistics
  - "average X" → DESCRIBE with mean
...
|---------|---------------------|-------------------|
| **Failure 1** (Multi-part query) | ❌ **No** - Needs LLM | Complex structure understanding ("count X and breakdown Y") requires semantic parsing |
| **Failure 2** (Filter extraction) | ✅ **Yes** - Pattern matching | Regex patterns: "not on X" → filter, "those patients with X" → filter, "for X" → filter |
| **Failure 3** (Categorical as numeric) | ✅ **Yes** - Code logic | Detect categorical encoding patterns: column names containing "1: Yes 2: No", "0: n/a 1: X", etc. |
| **Failure 4** (Query intent "how many") | ✅ **Yes** - Pattern matching | Regex: `r"how many"` → COUNT intent, `r"count"` → COUNT intent |
...
- **Normalize aliases**: Resolve "VL" → "viral_load" using persisted alias mappings (from ADR003)
- **Type-aware aggregation**: 
  - Categorical columns → frequency tables (not mean/median)
  - Numeric columns → descriptive statistics (mean, median, std dev)
  - Detects categorical encoding patterns ("1: Yes 2: No") and treats as categorical
- **Execute QueryPlan**: 
  - `QueryPlan(intent="COUNT", filters=[...])` → `query(metrics=["count"], filters={...})`
  - `QueryPlan(intent="DESCRIBE", metric="X")` → `query(metrics=["avg_X"])` or frequency table if categorical
  - `QueryPlan(intent="COMPARE_GROUPS", metric="X", group_by="Y")` → Grouped aggregation
- **Safe query compilation**: DuckDB/Ibis SQL generation with validation

**What Semantic Layer CANNOT Do** (Not Parser):
- ❌ Parse natural language intent ("how many" → COUNT)
- ❌ Infer multi-step plans from one sentence
- ❌ Handle negation logic reliably without structured input ("not on statin" requires NLU to map to column + value)
- ❌ Extract filters from query text (that's NLU's job)

**Architectural Split**:
- **NLU Layer (ADR001)**: Parses text → produces `QueryPlan`
- **Semantic Layer (ADR003)**: Validates `QueryPlan` → executes via SQL/Ibis → returns results
...
2. **Normalize Aliases** (including user-added aliases):
   - Resolve "VL" → "viral_load" using persisted alias mappings
   - Handle collisions (multiple columns match same alias)
   - Use canonical column names for execution

3. **Type-Aware Execution**:
   - **Categorical columns**: Detect encoding patterns ("1: Yes 2: No") → return frequency tables, not mean/median
...
4. **Refuse Invalid Plans**:
   - Unknown columns → raise `ValueError` with suggestions
   - Invalid operators → raise `ValueError` with supported operators
   - Type mismatches → raise `TypeError` with explanation
...
- Type-aware aggregation (categorical → frequency, numeric → stats)
- COUNT intent → use `query(metrics=["count"], filters={...})`
...
### Phase 4: Dependency on ADR001 Tier 3 LLM Hardening (P0 - Production Readiness)

**Scope Clarification**: ADR003 does NOT own LLM parsing implementation (that's ADR001's responsibility). This section documents the hardening requirements that ADR001's Tier 3 LLM fallback must implement for ADR003's execution contract to work reliably.

**Current State**: ADR001's Tier 3 LLM fallback is a stub (line 689 in `nl_query_engine.py`) that returns low-confidence DESCRIBE intent. Real-world failures (see Context section) demonstrate that complex queries require production-grade LLM parsing.

**Dependency**: ADR003's semantic layer execution contract (Phase 3) assumes ADR001 produces valid `QueryPlan` objects. If ADR001's Tier 3 fails silently or produces invalid plans, ADR003's execution will fail or produce incorrect results.

**Required Hardening Patterns** (ADR001 must implement):

#### 4.1 Structured JSON Extraction with Validation

**Requirement**: LLM responses must be structured JSON, not free-form text. Free-form parsing leads to "works in demo, lies in prod" failures.

**Implementation Pattern** (for ADR001):
- Use JSON mode (OpenAI) or structured output (Anthropic/Ollama)
- Parse JSON with validation (extract from markdown code blocks if needed)
- Validate against Pydantic schema before converting to `QueryIntent`
- Reject invalid responses (return `None`, don't fall back to default)

**Pydantic Schema** (ADR001 should use):
```python
from pydantic import BaseModel, Field
from typing import Literal

class LLMQueryExtraction(BaseModel):
    """Structured extraction from LLM response."""
    intent_type: Literal["COUNT", "DESCRIBE", "COMPARE_GROUPS", "FIND_PREDICTORS", "CORRELATIONS"]
...
**Note**: `Literal[...]` already constrains `intent_type` - no redundant validator needed. Pydantic handles validation automatically.

#### 4.2 Defense-in-Depth Validation

**Requirement**: Validate at multiple layers to catch errors early.

**Validation Layers** (ADR001 must implement):
1. **JSON Parsing**: Extract JSON from markdown code blocks, handle malformed JSON
2. **Schema Validation**: Pydantic validates types, ranges, required fields
3. **Semantic Layer Validation**: Check columns exist, operators valid, types compatible

**Filter Typing**: LLM boundary can use `list[dict[str, Any]]`, but convert to `FilterSpec` early for type safety:
```python
# LLM returns dict, convert to FilterSpec immediately
validated_filters = [FilterSpec(**f) for f in parsed["filters"]]
```

#### 4.3 LLM Client Abstraction (Provider-Independent)

**Requirement**: Core code must not directly instantiate provider SDKs (OpenAI, Anthropic). Use dependency injection for testability and provider swapping.

**Interface Pattern** (ADR001 should implement):
```python
from typing import Protocol

class LLMClient(Protocol):
    """Protocol for LLM clients (local or remote)."""
    def extract_intent_json(self, prompt: str, timeout_s: float) -> str:
        """Extract structured JSON from LLM. Raises LLMTimeoutError, LLMRateLimitError, etc."""
        ...

# Implementations:
# - NullLLMClient (disabled)
# - OllamaLLMClient (local)
# - RemoteLLMClient (OpenAI/Anthropic - future)
```

**Rationale**: Keeps `NLQueryEngine` deterministic and testable. Provider-specific logic isolated to client implementations.

#### 4.4 Error Handling and Retry Logic

**Requirement**: Production-grade error handling with retry logic for transient failures.

**Exception Hierarchy** (ADR001 should define):
```python
class LLMParseError(Exception):
    """Base exception for LLM parsing failures."""
    pass
...
class LLMInvalidResponseError(LLMParseError):
    """LLM returned invalid JSON or schema violation."""
    def __init__(self, response: str, validation_errors: list[str]):
        self.response = response
        self.validation_errors = validation_errors
```

**Retry Strategy**:
- Transient errors (timeout, rate limit): Retry with exponential backoff (max 2 retries)
- Permanent errors (invalid API key, malformed prompt): Fail immediately
- Log all retry attempts for observability

#### 4.5 Feature Flag and Confidence Gating

**Requirement**: LLM parsing must be optional and gated by confidence threshold. System must work without LLM.

**Configuration** (ADR001 should use):
```python
LLM_ENABLED = os.getenv("LLM_PARSING_ENABLED", "false").lower() == "true"
LLM_MODE = os.getenv("LLM_MODE", "local")  # "local" or "remote"
LLM_MODEL = os.getenv("LLM_MODEL", "llama3.1:8b-instruct")  # Ollama model
...
**Gating Logic**: Only use LLM result if `confidence >= LLM_CONFIDENCE_THRESHOLD`. Otherwise, fall back to lower tiers or return `None`.

#### 4.6 Observability and Logging

**Requirement**: All LLM calls must be logged with structured context for debugging at 3 AM.

**Required Log Fields**:
- `intent_type`, `confidence`, `elapsed_seconds`
- `filters_count`, `primary_variable`, `grouping_variable`
- `llm_provider`, `llm_model`, `retry_count`
- `error_type` (if failed), `validation_errors` (if invalid response)

**Token Tracking**: If provider SDK exposes token counts, log them. Do NOT use `len(prompt.split())` as token estimation - it's inaccurate.

#### 4.7 Privacy and PHI Protection

**Critical Requirement**: Do NOT send patient-level data to hosted LLM APIs.

**Context Building Rules** (ADR001 must follow):
- ✅ **Allowed**: Column names, aliases, data types, categorical value labels (from schema)
- ✅ **Allowed**: Column descriptions from metadata (if available)
- ❌ **Forbidden**: Sample row values, patient IDs, any PHI
- ❌ **Forbidden**: `_get_sample_values()` that returns actual data rows

**Rationale**: PHI in hosted LLM = privacy incident. Local LLM (Ollama) reduces risk but doesn't eliminate need for careful context building.

**Local LLM Considerations**:
- Privacy risk drops (data stays on-device), but still avoid unnecessary data exposure
- Latency becomes concern (CPU inference) - need strict timeouts
- Model quality usually worse than hosted - need tighter confidence gating
- Packaging complexity (ship weights or require user installation)

#### 4.8 Testing Requirements

**Requirement**: Mock-based unit tests for LLM parsing edge cases. No API calls in unit tests.

**Test Coverage** (ADR001 should implement):
- Multi-part query parsing (Failure 1)
- Filter extraction with negation (Failure 2)
- Invalid JSON handling (markdown code blocks, malformed JSON)
- Schema validation failures (invalid intent_type, out-of-range confidence)
- Timeout and retry logic
- Column validation (resolve aliases, check existence)

**Test Pattern**: Use `unittest.mock` to mock `LLMClient.extract_intent_json()`, test parsing/validation logic without API calls.

#### 4.9 MVP Cut Line (20/80 Rule)
...
- ✅ LLMClient interface (NullLLMClient, OllamaLLMClient)

**Skip for Now** (P1+):
- ❌ Sample values RAG (PHI risk)
- ❌ Type coercion beyond basic checks
- ❌ Multi-provider logic (remote LLM)
- ❌ Token cost accounting
- ❌ Advanced RAG context building (defer to ADR004)

**Rationale**: 80% of value comes from structured extraction + validation. Advanced features can wait.

#### 4.10 Coordination with ADR003

**Dependency Contract**: ADR003's semantic layer execution (Phase 3) assumes:
- ADR001 produces valid `QueryPlan` objects (not `None` unless all tiers fail)
- `QueryPlan.filters` are `list[FilterSpec]` (not dicts)
- `QueryPlan.intent` is one of valid intent types
- Column names in `QueryPlan` exist in semantic layer (after alias resolution)

**Failure Handling**: If ADR001's Tier 3 produces invalid `QueryPlan`, ADR003's execution will:
- Validate and reject with clear error message
- Log validation failure for debugging
- Return error to UI (don't silently fail)

**Success Criteria** (for ADR001 Tier 3):
- [ ] Structured JSON extraction with Pydantic validation
- [ ] LLMClient interface (no direct provider SDK instantiation)
- [ ] Timeout + retry logic for transient failures
- [ ] Feature flag + confidence gating
- [ ] Structured logging with observability fields
- [ ] No PHI in LLM context (column names/aliases only)
- [ ] Unit tests with mocked LLMClient (≥80% coverage)
- [ ] Handles multi-part queries (Failure 1) with confidence >0.75
- [ ] Extracts filters with negation (Failure 2)

**Note**: ADR003 does NOT implement Tier 3 LLM parsing. This section documents requirements that ADR001 must meet for ADR003's execution contract to work reliably.
...
- **Implementation**: Use LLMClient interface pattern (see Phase 4.3) - local vs remote is implementation detail
- **Requirement**: Same hardening patterns apply (structured JSON, validation, timeouts, confidence gating)
- **Decision**: Support local LLM via Ollama, but keep it optional and gated by feature flag
...
- `src/clinical_analytics/ui/pages/3_💬_Ask_Questions.py` - Result rendering (target for verification UI)
