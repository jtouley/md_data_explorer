...
| **Failure 2** (Filter extraction) | Pattern matching could work | Plan validation rejects plans with missing filters |
| **Failure 3** (Categorical as numeric) | Pattern matching could detect encoding | Type-aware execution (Phase 3) handles correctly regardless |
| **Failure 4** (COUNT intent) | Pattern matching could classify | COUNT intent execution (Phase 3) handles correctly regardless |

**ADR003 Mitigation Strategy**:
1. **Trust UI (Phase 1)**: Show parser output so users can verify correctness
2. **Alias Persistence (Phase 2)**: Teach system terminology to improve future matches
3. **Type-Aware Execution (Phase 3)**: Handle categorical variables correctly even when parser misclassifies
4. **Plan Validation (Phase 3)**: Reject invalid plans, log failures for debugging
...
**Known Failure Modes**: Real-world production queries (documented in Context section) demonstrate that complex clinical queries fail with current parser:
...
**ADR003 Mitigations**: This ADR mitigates parser limitations through:

1. **Plan Validation and Refusal** (Phase 3):
...
2. **Trust UI with Explicit Plan Inspection** (Phase 1):
   - Show `QueryPlan` (intent, metric, filters) in verification UI
...
3. **Alias Persistence to Improve Future Matches** (Phase 2):
...
- If ADR001 produces invalid `QueryPlan`, ADR003's execution will validate and reject with clear error message
