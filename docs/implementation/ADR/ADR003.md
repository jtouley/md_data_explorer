...
**Why This Matters**: This query demonstrates why trust UI (Phase 1) is critical - users need to verify the parser selected the correct variable and applied filters. Alias persistence (Phase 2) helps by teaching "LDL" → "LDL mg/dL" for future queries.
...
4. **Refuse Invalid Plans**:
   ...
   - **Contract violations**: If `requires_filters=True` but `filters=[]` → raise `ValueError` with explanation
...
# - requires_filters=True and filters not empty ✓
# - entity_key specified ✓
# - scope="filtered" with filters present ✓
...
- **Show Effective Execution**:
  ...
  - Applied filters (post-normalization, with actual values)
  - **Cohort size**:
...
- **Confidence Gating** (hard gate):
  - If `plan.confidence < confidence_threshold`: Refuse execution, return error dict with `requires_confirmation=True`
  - No side effects: Do NOT cache, store, or execute when below threshold
  - UI must show plan with confidence score, require explicit confirmation
- **Validate QueryPlan Contract**:
  ...
  - **COUNT-specific validation**:
    ...
    - **Breakdown validation**: If `grouping_variable=entity_key` (e.g., `patient_id`), refuse with error "Cannot group by entity key. Did you mean to group by a categorical field?"
...
- **Type-aware aggregation**: Execute safely for variables specified in plan (categorical → frequency, numeric → stats)
- **COUNT intent**: Use `query(metrics=["count"], filters={...}, entity_key=...)` with scope validation
- **Refuse invalid plans**: Contract violations → clear error messages
- **Output formatting**: Always compute both `count_total` (denominator) and `count_filtered` (numerator) for percentage reporting
- **Run-key generation**: Generate deterministic `run_key = hash(dataset_version + canonical_plan_json)` where canonical plan includes all fields (intent, variables, filters, entity_key, scope)
...
- Tier 3: LLM fallback (stub - returns low-confidence DESCRIBE intent) ⚠️
...
- **Multi-part queries** ("count X and breakdown Y") → Tier 3 stub returns low confidence
- **Filter extraction with negation** ("not on statin") → Not parsed, filters missing
- **COUNT intent** ("how many") → Misclassified as DESCRIBE, wrong statistics computed
- **Categorical variables** (encoded as 1/2) → Treated as numeric, meaningless statistics
...
- `confidence=0.3, tier=llm_fallback` → `analysis_execution_triggered` → `analysis_computation_complete`
- Low-confidence parses execute without user confirmation

**Contract Violation**: System promises "only auto-run when confidence ≥ threshold, otherwise require confirmation" but executes anyway.

**ADR003 Requirement**:
- **Hard gate**: `auto_execute := confidence >= threshold AND is_complete`
- **No side effects**: When `confidence < threshold`, do NOT:
  - Execute analysis computation
  - Cache results
  - Store results in session state
- **UI behavior**: Show parsed plan with confidence score, require explicit "Confirm and Run" button
- **Validation**: Executor must check confidence before execution, refuse if below threshold
...
**Observed Behavior** (from logs):
- `"average BMI of patients"` and `"average ldl of all patients"` → Same `run_key=...b7a5a863...`
...
**ADR003 Requirement**:
- **Run-key contract**: `run_key = hash(dataset_version + canonical_plan_json)` where canonical plan includes:
  - `intent_type`
  - `primary_variable` (canonical name, after alias resolution)
  - `grouping_variable` (canonical name, after alias resolution)
  - `filters` (normalized, sorted)
  - `entity_key` (for COUNT)
  - `scope` (for COUNT)
- **UI binding**: Each result panel must be bound to its specific QueryPlan (not generic cached result)
- **Validation**: Executor must generate deterministic run_key from complete plan, UI must verify plan matches before displaying cached results
...
- Result: Giant list of patient IDs instead of statin × regimen breakdown
...
8. **Alias Persistence to Improve Future Matches** (Phase 2):
   - User teaches system terminology ("VL" → "viral_load")
