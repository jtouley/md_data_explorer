...
**Example Scenario**:
- Doctor uploads `patient_data.csv` as "Study A" → creates dataset "Study A"
- Doctor uploads same `patient_data.csv` file as "Study B" → creates duplicate dataset "Study B"
...
- ✅ **Add nullable columns only**: New columns added, all existing columns preserved
- ✅ **No type changes**: All existing columns keep same types
- ✅ **No column removals**: All existing columns remain

**Blocked Drift (Requires User Override):**
- ❌ **Column removals**: Any column removed from schema
- ❌ **Type changes**: Any column type changed (e.g., `Int64` → `Float64`)
- ❌ **Rename without mapping**: Column removed and new column added (possible rename) without explicit rename mapping
...
def detect_schema_changes(old_schema: dict, new_schema: dict) -> dict:
    ...
    return {
        ...
        "type_changes": [f"{tc['column']}: {tc['old_type']} → {tc['new_type']}" for tc in drift["type_changes"]],
...
def check_schema_compatibility(current_schema: dict, target_schema: dict) -> dict:
    ...
    for col in common_cols:
        ...
        if current_type != target_type:
            type_changes.append(f"{col}: {current_type} → {target_type}")
        ...
        if current_version and current_version["version"] != target_version:
            ...
            if not compatibility["is_backward_compatible"]:
                if compatibility.get("removed_columns"):
                    warnings.append(f"⚠️ Columns removed in target version: {compatibility['removed_columns']}")
                if compatibility.get("type_changes"):
                    warnings.append(f"⚠️ Type changes in target version: {compatibility['type_changes']}")
...
[ ] Add tests for cross-dataset content deduplication
    - Same content, different name → blocked with helpful error
    - Same content, same name, overwrite=True → handled by overwrite logic
    - Different content, different name → allowed (new dataset)
